...
if __name__ == "__main__":            
    ...
    train_env = CustomEnv(train_df, lookback_window_size=lookback_window_size)
    train_agent(train_env, agent, visualize=False, train_episodes=50000, training_batch_size=500)
    #test_env = CustomEnv(test_df, lookback_window_size=lookback_window_size, Show_reward=True)
    #test_agent(test_env, agent, visualize=True, test_episodes=10, folder="2021_01_11_23_48_Crypto_trader", name="..._Crypto_trader", comment="")
    '''

    '''
    #With LSTM ele nao tem este modelo treinado teria de executa-lo para o treinar e depois testar
    agent = CustomAgent(lookback_window_size=lookback_window_size, lr=0.00001, epochs=1, optimizer=Adam, batch_size = 128, model="LSTM")
    train_env = CustomEnv(train_df, lookback_window_size=lookback_window_size)
    train_agent(train_env, agent, visualize=False, train_episodes=50000, training_batch_size=500)
    #test_env = CustomEnv(test_df, lookback_window_size=lookback_window_size, Show_reward=True)
    #test_agent(test_env, agent, visualize=True, test_episodes=10, folder="2021_01_11_23_43_Crypto_trader", name="..._Crypto_trader", comment="")
    '''
